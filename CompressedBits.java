...
public class CompressedBits {
	...
	// call this method in SimpleHuffProcessor to return the array of frequency of
	// the file
	public int[] getArrayFreq() throws IOException {
		return wordFreq;
	}

	// helper method to create the array of frequency using information from
	// InputStream
	...
	// helper method to create the tree for the file using information from the
	...
	private void createTree() {
		...
		// adding --> removing --> add again into the queue until there is only 1 node
		// left (tree node);
		while (queue.size() > 1) {
	...
	// helper method to recurse through the tree of the file and creating strings
	// that represents
	...
	// getting the total number that would represent the tree 
	// [tree size + (# of leaf nodes * 9)]
